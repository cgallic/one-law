...
function Ground() {
  return (
    <group>
      <mesh receiveShadow rotation-x={-Math.PI / 2} position-y={-0.12}><boxGeometry args={[42, 31, 0.25]} /><meshStandardMaterial color="#313432" roughness={0.9} /></mesh>
      ...
      <mesh position={[0, 0.01, -12.7]}><boxGeometry args={[40, 0.08, 1.5]} /><meshStandardMaterial color="#09242b" roughness={0.25} metalness={0.35} /></mesh>
      <mesh position={[0, 0.035, -12.7]} rotation-x={-Math.PI / 2}><planeGeometry args={[39, 1.25]} /><meshStandardMaterial color="#0b3944" emissive="#06303b" emissiveIntensity={0.35} /></mesh>
    </group>
  );
}
...
function Districts({ state }: Props) {
  const buildings = useMemo<Building[]>(() => Array.from({ length: 76 }, (_, index) => {
    ...
    return { x, z, w: 1.2 + seeded(index, 7) * 1.05, d: 1.1 + seeded(index, 9) * 0.8, h: 1.2 + seeded(index, 11) * (district === "civic" ? 4.5 : 3), district };
  }), []);
...
function Scene({ state }: Props) {
  ...
  return (
    <>
      ...
      <Ground /><TerritoryTiles state={state} /><Districts state={state} /><Forum trust={state.trust} /><CivicDetails state={state} /><Trees equality={state.equality} /><Transit stability={state.stability} /><Citizens population={state.population} liberty={state.liberty} /><ControlSystem state={state} />
...
export default function WorldCanvas({ state }: Props) {
  ...
  return <Canvas shadows dpr={[1, 1.6]} camera={{ position:[25,24,30], fov:38, near:0.1, far:120 }} gl={{ antialias:true, powerPreference:"high-performance" }}><Scene state={state} /></Canvas>;
}
