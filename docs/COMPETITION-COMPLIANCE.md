# OpenAI Build Week competition compliance

Audited July 18, 2026 against the official rules, FAQ, and judging criteria.

## Stage-one eligibility

| Requirement | Evidence | Status |
|---|---|---|
| Built with Codex | Dated Git commits, README collaboration narrative, primary build thread | Pass; `/feedback` ID must be entered on Devpost |
| Meaningful GPT-5.6 use | `src/app/api/worlds/compile/route.ts`, strict schema, constitution drives deterministic law consistency | Code complete; production key required for live evidence |
| Education track | AI-literacy/civics learning goal, playable specification failure, classroom debrief | Pass |
| Functional and consistent | Public Vercel routes, cached continuity mode, 20 automated tests, browser acceptance | Pass |
| New during submission period | Repository root commit and all implementation commits dated July 18, 2026 | Pass |
| Authorized third-party software | MIT/open-source dependencies documented separately | Pass |

## Required submission materials

| Material | Status |
|---|---|
| English text description explaining features | Ready in `docs/DEVPOST.md` |
| Public YouTube video, three minutes or less | Script ready; recording/upload required |
| Clear audio explaining product, Codex, and GPT-5.6 | Covered in `docs/DEMO-SCRIPT.md`; recording required |
| No unlicensed trademarks, music, or footage | Product uses original code geometry and procedural sound |
| Public repository with relevant license | `https://github.com/cgallic/one-law`, MIT |
| README setup/testing instructions | Complete |
| README Codex collaboration and key decisions | Complete |
| Working free judge link | `https://one-law-theta.vercel.app/demo` |
| `/feedback` primary-thread Session ID | Entrant must run `/feedback` in the primary build thread |
| Devpost registration and Education category | Entrant action required |

## Judging optimization

### Technological implementation

- Non-trivial deterministic simulation with bounded causal effects, factions, support normalization, signed state, replay, and five ordered endings.
- GPT-5.6 owns semantic interpretation while Zod and application code own truth and causality.
- 20 automated tests plus production and browser acceptance.

### Design

- Complete authority-to-judgment dramatic arc rather than a proof of concept.
- Real-time inhabited 3D civic world with physical consequences, accessible controls, reduced motion, non-WebGL fallback, sound toggle, persistence, and export.

### Potential impact

- Specific audience: secondary-school and university AI-literacy, civics, ethics, and policy classrooms.
- The result includes three discussion prompts addressing specification, precedent, and alignment without profiling the player.
- Five-playtest report template records whether players understand causal learning.

### Quality of idea

- The model obeys rather than rebels; the personal reveal comes from the gap between declared and operative values.
- Every shadow clause cites a player ruling and every ending is earned from deterministic state.

## Hard blockers before submission

1. Add a funded `OPENAI_API_KEY` to Vercel Production and Preview and verify one live GPT-5.6 compilation.
2. Run `/feedback` in the primary Codex build thread and copy the Session ID.
3. Record the scripted demo with English voiceover, keep it at or below 3:00, upload publicly to YouTube, and paste its URL.
4. Register/submit on Devpost in Education before July 21, 2026 at 5:00 PM Pacific.
5. Keep the public game and repository free and available through the judging period.
